...
class DecoderSurface implements SurfaceTexture.OnFrameAvailableListener {
     ...
     */
    void drawImage(final float elapsedTime) {
...
//                    Log.d(TAG, "scale[0] = " + scale[0] + " scale[1] = " + scale[1]);
        ...
        if (filter != null) {
            ...
            framebufferObject.enable();
            GLES20.glClear(GL_COLOR_BUFFER_BIT);
            filter.draw(filterFramebufferObject.getTexName(), framebufferObject, elapsedTime);
        }
        ...
        normalShader.draw(framebufferObject.getTexName(), null, elapsedTime);
    }
